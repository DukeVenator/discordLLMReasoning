...
import { Client, GatewayIntentBits, Partials, Message, ChannelType, EmbedBuilder, TextChannel, DMChannel, NewsChannel } from 'discord.js'; // Consolidated imports + DMChannel, NewsChannel
...
import { BaseProvider, ChatMessage, ChatMessageContentPart, ChatMessageContentPartText, GenerationOptions, StreamChunk } from '../providers/baseProvider'; // Added GenerationOptions, StreamChunk
   ...
   */
    ...
    public async initialize(): Promise<void> {
        ...
        try {
            ...
            this.logger.info(`Configuration loaded successfully. Logger level set to: ${this.logger.getLevel()}`); // Use getLevel()
        ...
        try {
            ...
            this.logger.info(`Default LLM provider initialized: ${this.llmProvider.getProviderInfo ? JSON.stringify(this.llmProvider.getProviderInfo()) : 'Info N/A'}`);
        } catch (error: any) {
            this.logger.error(`Failed to initialize provider factory or default provider: ${error.message}`);
        ...
        try {
            ...
            this.logger.info(`Tool registry initialized with ${this.toolRegistry.getToolDefinitions().length} tools.`);
        } catch (error) {
        ...
        if (this.config.memory.enabled) {
            try {
                ...
                if (!dbPath) {
                    throw new Error('SQLite database path is not defined in the configuration (memory.sqlite.path).');
                }
                // Pass config and providerFactory to the constructor
                this.memoryStorage = new SQLiteMemoryStorage(dbPath, this.logger, this.config, this.providerFactory);
            ...
            if (this.memoryStorage) {
                try {
                    ...
                    this.memoryCommandHandler = new MemoryCommandHandler(this.memoryStorage, this.logger); // Removed config argument
                    this.logger.info('Memory command handler initialized.');
                } catch (error) {
                    this.logger.error('Failed to initialize memory command handler:', error);
                    this.memoryCommandHandler = null; // Ensure it's null on error
                }
            } else {
                this.logger.warn('Memory command handler not initialized because memory storage is unavailable.');
        ...
        if (this.config.reasoning?.enabled) {
            try {
                ...
                this.reasoningManager = new ReasoningManager(this, this.providerFactory);
            } catch (error) {
     ...
     * @private
     */
    private async processMessageNode(message: Message, node: MessageNode): Promise<void> {
        node.messageId = message.id;
        node.role = message.author.bot && message.author.id === this.client.user?.id ? 'assistant' : 'user';
        ...
        if (botMention && message.channel.type !== ChannelType.DM && cleanedContent.startsWith(botMention)) {
        ...
        const providerSupportsVision = this.llmProvider ? this.llmProvider.supportsVision() : false; // Check provider capability dynamically
        const maxAttachmentSize = this.config.llm?.maxAttachmentSizeBytes ?? 10 * 1024 * 1024; // Default 10MB, make configurable
        ...
        if (message.attachments.size > 0 && providerSupportsVision) {
            ...
            this.logger.debug(`[Node ${message.id}] Processing ${message.attachments.size} attachments...`);

            for (const attachment of message.attachments.values()) {
                const fileExtension = attachment.name?.split('.').pop()?.toLowerCase();
                const isSupportedType = attachment.contentType?.startsWith('image/') && fileExtension && supportedImageTypes.includes(fileExtension);
                ...
                if (isSupportedType && isWithinSizeLimit) {
                    try {
                        ...
                        if (!attachment.contentType) {
                            ...
                            this.logger.warn(`[Node ${message.id}] Ignoring attachment ${attachment.name} due to missing content type.`);
                            continue; // Skip to the next attachment
                        }

                        this.logger.debug(`[Node ${message.id}] Fetching image: ${attachment.name} (${attachment.contentType}, ${attachment.size} bytes) from ${attachment.url}`);
                        ...
                        if (response.status === 200 && response.data) {
                            ...
                            this.logger.debug(`[Node ${message.id}] Successfully fetched and encoded image: ${attachment.name}`);
                        } else {
                            throw new Error(`Received status ${response.status}`);
                        }
                    } catch (error: any) {
                        node.hasBadAttachments = true;
                        this.logger.warn(`[Node ${message.id}] Failed to fetch or encode image ${attachment.name} from ${attachment.url}: ${error.message}`);
                    }
                } else {
                    node.hasBadAttachments = true;
                    if (!isSupportedType) {
                        this.logger.debug(`[Node ${message.id}] Ignoring unsupported attachment type: ${attachment.name} (${attachment.contentType ?? 'N/A'})`);
                    } else if (!isWithinSizeLimit) {
                        this.logger.debug(`[Node ${message.id}] Ignoring attachment due to size limit: ${attachment.name} (${attachment.size} > ${maxAttachmentSize} bytes)`);
                    }
                }
            }
            ...
            this.logger.debug(`[Node ${message.id}] Ignoring ${message.attachments.size} attachments as provider does not support vision.`);
        }
        ...
        if (message.reference?.messageId) {
            try {
                node.parentMessage = await message.channel.messages.fetch(message.reference.messageId);
            } catch (error) {
                node.fetchParentFailed = true;
                this.logger.warn(`[Node ${message.id}] Failed to fetch parent message ${message.reference.messageId}: ${error}`);
            }
        }
         this.logger.debug(`[Node ${message.id}] Processed: role=${node.role}, images=${node.images.length}, parent=${node.parentMessage?.id ?? 'None'}, text=${(node.text ?? '').substring(0,50)}...`);
    }
     ...
     */
    public async buildMessageHistory(latestMessage: Message): Promise<[ChatMessage[], Set<string>]> {
        ...
        this.logger.debug(`[History] Building for ${latestMessage.id} (max_messages=${maxMessages}, max_images=${maxImages})`);
        ...
        while (currentMessage && llmHistory.length < maxMessages) {
            ...
            if (!node) {
                isNewNode = true;
                node = {
                    text: null, images: [], role: 'user', userId: null,
                    hasBadAttachments: false, fetchParentFailed: false,
                    parentMessage: null, messageId: messageId,
                };
                this.logger.debug(`[History] Processing new node for message ${messageId}`);
                await this.processMessageNode(currentMessage, node);
                this.messageNodeCache.set(messageId, node);
            } else {
                this.logger.debug(`[History] Using cached node for message ${messageId}`);
            }
            ...
            let userName: string | undefined = undefined;
                        ...
                        const providerSupportsUsernames = this.llmProvider ? this.llmProvider.supportsUsernames() : false;
                        const providerSupportsVision = this.llmProvider ? this.llmProvider.supportsVision() : false; // Use dynamic check
            let userPrefix = "";
            if (node.role === 'user' && !providerSupportsUsernames) {
                const safeDisplayName = currentMessage.author.displayName.replace(/[^a-zA-Z0-9 _-]/g, '');
            ...
            if (imagesToSend.length > 0 && providerSupportsVision) {
                ...
                this.logger.debug(`[History] Formatted node ${messageId} as multimodal (${imagesToSend.length} images)`);
            } else {
                apiContent = prefixedTextToSend;
            }

            if ((typeof apiContent === 'string' && apiContent) || (Array.isArray(apiContent) && apiContent.length > 0)) {
                 // Assign content directly (type matches now)
                 const messageEntry: ChatMessage = { role: node.role, content: apiContent };
                 // Add name property if it exists
                 if (userName) { messageEntry.name = userName; }
                 llmHistory.push(messageEntry);
            } else if (isNewNode) {
                 this.logger.debug(`[History] Node ${messageId} resulted in empty content after formatting.`);
            }

            // --- Collect Warnings ---
            if ((node.text?.length ?? 0) > maxTextLength) {
                userWarnings.add(`⚠️ Max ${maxTextLength.toLocaleString()} characters/message`);
            }
            if (node.images.length > maxImages) {
                // Handle maxImages = 0 case specifically
                userWarnings.add(maxImages === 0 ? '⚠️ Max 0 images/message' : `⚠️ Max ${maxImages} image${Number(maxImages) === 1 ? '' : 's'}/message`);
            }
            ...
            if (node.fetchParentFailed) {
                ...
                userWarnings.add(`⚠️ Using last ${llmHistory.length} message${Number(llmHistory.length) === 1 ? '' : 's'}`);
            }

            // Move to parent
            currentMessage = node.parentMessage;
            if (currentMessage) {
                this.logger.debug(`[History] Moving to parent message ${currentMessage.id}`);
            }
        } // End while loop

        llmHistory.reverse();
        this.logger.debug(`[History] Final history size: ${llmHistory.length} messages`);
    ...
    private registerEventHandlers(): void {
        ...
        this.client.on('messageCreate', (message) => onMessageCreate(this, message));
    ...
    public async shutdown(): Promise<void> {
        ...
        try {
            ...
            if (this.client.readyTimestamp) { // Check if client is still potentially connected
                 this.client.destroy();
     ...
     * @private
     */
    private _processMemorySuggestions(userId: string, responseText: string, messageId: string): string {
        ...
        if (this.memoryStorage && this.config.memory.enabled) {
            ...
            const appendRegex = new RegExp(`${appendStartEsc}([\\s\\S]*?)${appendEndEsc}`, 'gi');
            const replaceRegex = new RegExp(`${replaceStartEsc}([\\s\\S]*?)${replaceEndEsc}`, 'gi');
            ...
            if (replaceMatches.length > 0) {
                ...
                this.logger.info(`[${messageId}] Found memory replace suggestion for user ${userId}.`);
                if (replaceContent) {
                    this.memoryStorage.setMemory(userId, replaceContent); // Use setMemory
                }
            } else if (appendMatches.length > 0) { // Only process append if replace wasn't found
                tagsProcessed = true;
                ...
                this.logger.info(`[${messageId}] Found memory append suggestion for user ${userId}.`);
                if (appendContent) {
            ...
            if (stripTags && tagsProcessed) {
                ...
                const combinedStripRegexWithSpace = new RegExp(`\\s*(?:${appendStartEsc}[\\s\\S]*?${appendEndEsc}|${replaceStartEsc}[\\s\\S]*?${replaceEndEsc})\\s*`, 'gi');
                // Replace tags and surrounding space with a single space to avoid joining words
                processedResponse = processedResponse.replace(combinedStripRegexWithSpace, ' ');
                // Collapse multiple spaces into one and trim ends
                processedResponse = processedResponse.replace(/\s{2,}/g, ' ').trim();
                this.logger.debug(`[${messageId}] Stripped memory suggestion tags from final response.`);
            } else if (tagsProcessed) {
                 this.logger.debug(`[${messageId}] Memory suggestion tags processed but configured not to strip.`);
            }
        }
...
// Removed duplicated helper functions
    ...
    public async processMessage(message: Message): Promise<void> {
        this.logger.info(`Processing message ID ${message.id} from ${message.author.tag} in channel ${message.channelId}`);
        ...
        if (this.config.memory.enabled && this.memoryStorage) {
            try {
                ...
                this.logger.debug(`[${message.id}] Fetched memory for user ${userId}. Length: ${userMemoryContent?.length ?? 0}`);
            } catch (memError) {
                this.logger.error(`[${message.id}] Failed to retrieve memory for user ${userId}:`, memError);
        ...
        if (history.filter(msg => msg.role !== 'system').length === 0) { // Check if only system/memory messages remain
            this.logger.warn(`[${message.id}] No user/assistant content found in message or its history (after memory injection). Aborting.`);
            if (userWarnings.size > 0) {
                try {
                    ...
                    await message.reply(`Processing stopped: ${stopReason}${combinedWarnings ? ` (${combinedWarnings})` : ''}`);
                } catch (replyError) {
                    this.logger.error(`[${message.id}] Failed to send warning reply:`, replyError);
                }
            } else {
                 try {
                    ...
                    this.logger.error(`[${message.id}] Failed to send stop notification reply:`, replyError);
                 }
            }
            return;
        }
...
// Removed old memory injection block (now handled above)
        ...
        let baseSystemPromptText = this.config.llm?.defaultSystemPrompt ?? "You are LLMCord, a helpful Discord bot.";

        // --- Format Memory Separately ---
        const formattedMemoryForPrompt = this._formatMemoryForSystemPrompt(userMemoryContent);
        ...
        if (this.reasoningManager?.isEnabled()) {
             const reasoningSignal = this.config.reasoning?.signalStart ?? '[USE_REASONING_MODEL]'; // Use configured signal
             ...
             this.logger.debug(`[${message.id}] Appended reasoning trigger instructions.`);
        }
        ...
        const updateIntervalMs = this.config.discord?.streamingUpdateIntervalMs ?? 1500; // Default 1.5 seconds
        ...
        const updateDiscordResponse = async (content: string, isFinal: boolean = false): Promise<void> => {
            const now = Date.now();
            ...
            if (!isFinal && content.length - (currentReplyMessage?.content ?? currentReplyMessage?.embeds[0]?.description ?? '').length < minContentLengthChange) {
                 return;
            }
            ...
            if (contentToSend.length > characterLimit) {
                ...
                contentToSend = contentToSend.substring(0, characterLimit);
                 this.logger.warn(`[${message.id}] Response chunk exceeded limit (${characterLimit}). Splitting message.`);
            }
            ...
            try {
                ...
                const finalDescription = isFinal ? descriptionText : `${descriptionText} ⚪`;
                ...
                if (isFinal && !contentToSend.trim()) {
                    ...
                    this.logger.debug(`[${message.id}] Final response is empty, sending completion indicator.`);
                    ...
                    messagePayload = usePlainResponses
                        ...
                        : { embeds: [embed.setDescription(finalDescription).setColor(finalColor)] };
                }
                ...
                if (!currentReplyMessage) {
                    ...
                    this.logger.debug(`[${message.id}] Sent initial streaming message ${currentReplyMessage.id}`);
                } else {
                    // Edit existing message
                    await currentReplyMessage.edit(messagePayload);
                    this.logger.debug(`[${message.id}] Edited streaming message ${currentReplyMessage.id}`);
                }
                ...
                if (remainingContent !== null) {
                    ...
                    if (message.channel.isTextBased() && 'send' in message.channel && remainingContent) { // Ensure remainingContent is not null/empty
                       const followUpEmbed = new EmbedBuilder();
                       const followUpDescription = isFinal ? remainingContent : `${remainingContent} ⚪`;
                       ...
                       const followUpPayload = usePlainResponses
                           ...
                           : { embeds: [followUpEmbed.setDescription(followUpDescription).setColor(followUpColor)] };
                       try {
                           ...
                           const followUpMessage = await (message.channel as TextChannel | DMChannel | NewsChannel).send(followUpPayload);
                           currentReplyMessage = followUpMessage; // Update the message being edited
                           replyMessages.push(followUpMessage);
                           this.logger.debug(`[${message.id}] Sent follow-up streaming message ${followUpMessage.id}`);
                       } catch (sendError) {
                            this.logger.error(`[${message.id}] Failed to send follow-up message:`, sendError);
                            // Decide how to handle this - maybe truncate? For now, log and potentially lose data.
                       }
                   } else {
                        this.logger.warn(`[${message.id}] Cannot send follow-up message in channel type: ${message.channel.type} (not text-based or lacks send method)`);
                        // Log and potentially lose data if channel doesn't support send
                   }
                }
            } catch (error: any) {
                 if (error.code === 50035 && error.message.includes('longer than')) { // DiscordAPIError[50035]: Invalid Form Body (content: Must be 2000 or fewer in length.)
                     this.logger.warn(`[${message.id}] Content exceeded limit during edit/reply, likely race condition or calculation error. Truncating slightly.`);
                     // Attempt to truncate and retry ONCE. More complex retry logic could be added.
                     try {
                         const slightlyTruncatedContent = contentToSend.substring(0, characterLimit - 10); // Shave off a bit more
                         const truncatedPayload = usePlainResponses
                             ? { content: slightlyTruncatedContent || "..." }
                             : { embeds: [new EmbedBuilder().setDescription(slightlyTruncatedContent || "...")] };
                         if (currentReplyMessage) {
                         ...
                         this.logger.error(`[${message.id}] Failed to edit/reply even after truncating:`, retryError);
                     }

                 } else if (error.code === 429) { // Rate limited
                    this.logger.warn(`[${message.id}] Discord rate limit hit during streaming update. Skipping update.`);
                    ...
                    this.logger.error(`[${message.id}] Failed to update Discord response:`, error);
        ...
        try {
            ...
            if (modelSetting && modelSetting.includes('/')) {
                ...
                } else if ((providerName === 'gemini' || providerName === 'google' || providerName === 'google-gemini') && this.config.llm.gemini) {
                ...
                const tempFromConfig = providerConfig?.temperature ?? this.config.llm.defaultTemperature;
                if (tempFromConfig !== undefined) {
                    generationOptions.temperature = tempFromConfig;
                }
                const maxTokensFromConfig = providerConfig?.maxOutputTokens ?? this.config.llm.defaultMaxTokens;
                if (maxTokensFromConfig !== undefined) {
                    generationOptions.maxOutputTokens = maxTokensFromConfig;
                }

                this.logger.debug(`[${message.id}] Using base generation options: temp=${generationOptions.temperature ?? 'default'}, maxTokens=${generationOptions.maxOutputTokens ?? 'default'}`);

           } else {
                this.logger.warn(`[${message.id}] Could not parse provider name from config.model: "${modelSetting}". Using default generation parameters.`);
           ...
           // Add tools to options if the provider supports them
            if (this.llmProvider.supportsTools()) {
                 if (!generationOptions) { generationOptions = {}; } // Ensure options object exists
                 generationOptions.tools = this.toolRegistry.getToolDefinitions(); // Use ToolRegistry
                 this.logger.debug(`[${message.id}] Adding ${this.toolRegistry.getToolDefinitions().length} tools to generation options.`); // Use ToolRegistry
            }
            ...
            try {
                ...
                replyMessages.push(currentReplyMessage);
                this.logger.debug(`[${message.id}] Sent initial placeholder message ${currentReplyMessage.id}`);
            } catch (initialReplyError) {
                this.logger.error(`[${message.id}] Failed to send initial placeholder reply:`, initialReplyError);
            ...
            this.logger.debug(`[${message.id}] Base system prompt from config: ${baseSystemPromptText ? `'${baseSystemPromptText.substring(0, 50)}...'` : 'undefined'}`);
            this.logger.debug(`[${message.id}] Formatted memory for prompt: ${formattedMemoryForPrompt ? `'${formattedMemoryForPrompt.substring(0, 100)}...'` : 'None'}`);
            this.logger.debug(`[${message.id}] Instructions: ${instructions ? `'${instructions.substring(0, 100)}...'` : 'None'}`);

            const providerSupportsSystem = this.llmProvider.supportsSystemPrompt();
            this.logger.debug(`[${message.id}] Provider supports system prompt: ${providerSupportsSystem}`);

            if (providerSupportsSystem) {
                // Combine all parts for the system prompt argument
                finalSystemPrompt = (baseSystemPromptText + formattedMemoryForPrompt + instructions).trim();
                this.logger.debug(`[${message.id}] Passing history and combined system prompt to generateStream. SystemPrompt: ${finalSystemPrompt ? `'${finalSystemPrompt.substring(0, 100)}...'` : 'undefined'}`);
                // History already contains messages, memory was NOT injected here earlier
                stream = this.llmProvider.generateStream(historyToUse, finalSystemPrompt, generationOptions);
            } else {
                // Prepend system prompt, memory, and instructions to the first user message
                this.logger.debug(`[${message.id}] Provider does not support system role. Prepending prompt, memory, and instructions to first user message.`);
                const combinedPrefix = (baseSystemPromptText + formattedMemoryForPrompt + instructions).trim();
                const userMessageIndex = historyToUse.findIndex(msg => msg.role === 'user');
                ...
                if (userMessageIndex !== -1 && combinedPrefix) {
                    ...
                    if (userMessage) {
                        ...
                        this.logger.debug(`[${message.id}] Combined prompt prepended to message ${userMessageIndex}.`);
                    } else {
                        this.logger.warn(`[${message.id}] Found user message index but message object was undefined.`);
                    }
                } else if (combinedPrefix) {
                     this.logger.warn(`[${message.id}] Could not find user message to prepend combined prompt.`);
                }
                // Call generateStream without the systemPrompt argument
                this.logger.debug(`[${message.id}] Passing modified history to generateStream (prepended). History: ${JSON.stringify(historyToUse)}, SystemPrompt: undefined`);
                stream = this.llmProvider.generateStream(historyToUse, undefined, generationOptions);
            }
            ...
            for await (const chunk of stream) {
                ...
                if (chunk.toolCalls && chunk.toolCalls.length > 0) {
                    ...
                    this.logger.info(`[${message.id}] Tool call(s) detected in stream chunk: ${JSON.stringify(chunk.toolCalls)}`);
                    // Don't break yet, wait for isFinal
                }

                // Handle final chunk
                if (chunk.isFinal) {
                    this.logger.info(`[${message.id}] LLM Stream End. Reason: ${chunk.finishReason}. Total length: ${accumulatedResponse.length}. Tool Calls: ${!!toolCallsDetected}`);
                    // Final update for any remaining content
                    await updateDiscordResponse(accumulatedResponse, true);

                    // --- Tool Call Handling ---
                    if (toolCallsDetected) {
                        this.logger.info(`[${message.id}] Handling ${toolCallsDetected.length} tool call(s).`);
                        ...
                        historyToUse.push({ // Add to the history being used for the next call
                            role: 'assistant',
                        ...
                        this.logger.debug(`[${message.id}] Added placeholder user message ('${placeholderText}') to history for Gemini API compliance.`);
+
                        // 4. Call LLM again with updated history (including tool results and placeholder)
                        this.logger.info(`[${message.id}] Re-calling LLM after tool execution with updated history (${historyToUse.length} messages).`);
                        ...
                        this.logger.debug(`[${message.id}] History before second LLM call: ${JSON.stringify(historyToUse, null, 2)}`);
                        ...
                        if (providerSupportsSystem) {
                            ...
                            this.logger.debug(`[${message.id}] Passing history and system prompt for second call.`);
                        } else {
                            ...
                            this.logger.debug(`[${message.id}] Prepending combined prompt for second call.`);
                            const combinedPrefix = (baseSystemPromptText + formattedMemoryForPrompt + instructions).trim();
                        ...
                        for await (const secondChunk of secondStream) {
                            ...
                            if (secondChunk.toolCalls && secondChunk.toolCalls.length > 0) {
                                this.logger.warn(`[${message.id}] Nested tool calls detected in second LLM response. This is not fully supported. Ignoring.`);
                            }
                            if (secondChunk.isFinal) {
                                this.logger.info(`[${message.id}] Second LLM Stream End. Reason: ${secondChunk.finishReason}. Final length: ${accumulatedResponse.length}.`);
            ...
            this.logger.debug(`[${message.id}] Full LLM Response processed (length: ${accumulatedResponse.length})`);
            ...
            const responseToCheckForMemory = finalResponseToUser || accumulatedResponse;
            finalResponseToUser = this._processMemorySuggestions(userId, responseToCheckForMemory, message.id);
            ...
            if (this.reasoningManager?.isEnabled() && finalResponseToUser) {
                 ...
                 if (this.reasoningManager.checkResponseForSignal(finalResponseToUser)) {
                     if (!this.reasoningManager.checkRateLimit(userId)) {
                         const signal = this.reasoningManager.getReasoningSignal(finalResponseToUser);
                         if (signal) {
                             try {
                                 this.logger.info(`[${message.id}] Invoking reasoning manager for user ${userId}...`);
                                 ...
                                 const reasoningResult = await this.reasoningManager.generateReasoningResponse(history, signal, userId); // Pass original history
                                 if (reasoningResult.shouldProcess && reasoningResult.finalResponse) {
                                     this.logger.info(`[${message.id}] Reasoning manager provided a final response.`);
                                     // Update finalResponseToUser with the reasoning result
                                     finalResponseToUser = reasoningResult.finalResponse;
                                 } else {
                                     this.logger.info(`[${message.id}] Reasoning manager processed but did not provide a final response. Falling back.`);
                                      // Remove the reasoning tag even if no final response is provided
                                      finalResponseToUser = finalResponseToUser.replace(/\[REASONING_REQUEST\][\s\S]*?\[\/REASONING_REQUEST\]/gi, '').trim();
                                 }
                             } catch (reasoningError) {
                                 this.logger.error(`[${message.id}] Error during reasoning process:`, reasoningError);
                                 // Remove the reasoning tag on error
                                 finalResponseToUser = finalResponseToUser.replace(/\[REASONING_REQUEST\][\s\S]*?\[\/REASONING_REQUEST\]/gi, '').trim();
                             }
                         } else {
                              this.logger.warn(`[${message.id}] Reasoning signal detected but could not be extracted.`);
                              // Remove the reasoning tag if extraction fails
                              finalResponseToUser = finalResponseToUser.replace(/\[REASONING_REQUEST\][\s\S]*?\[\/REASONING_REQUEST\]/gi, '').trim();
                         }
                     } else {
                          this.logger.warn(`[${message.id}] Reasoning request rate limited for user ${userId}.`);
            ...
            const warningsString = userWarnings.size > 0 ? `\n\n*(${Array.from(userWarnings).join(', ')})*` : '';
            const finalContentWithWarnings = (finalResponseToUser || (warningsString ? "" : "No response generated.")) + warningsString; // Ensure something is sent
            ...
            if (finalContentWithWarnings.trim()) {
                 ...
                 this.logger.info(`[${message.id}] No content left to send after processing tags and adding warnings.`);
                 // The logic to handle empty final messages is now inside updateDiscordResponse
            }

        } catch (error: any) { // Keep the : any type from the actual file content
            this.logger.error(`[${message.id}] Error processing message with LLM:`, error); // Keep the exact error message
            try {
                // Attempt to send an error message back to the user
                await message.reply("Sorry, I encountered an error while processing your request."); // Keep the exact reply text
            } catch (replyError) {
                this.logger.error(`[${message.id}] Failed to send error reply to Discord:`, replyError); // Keep the exact error message
            }
        }
    } // End processMessage
     ...
     * @private
     */
    private async _executeToolCall(toolCall: ToolCallRequest): Promise<string> { // Return type is string for LLM
        this.logger.info(`Executing tool: ${toolCall.toolName} with ID: ${toolCall.id}`);
        let result: any;
        try {
            ...
            this.logger.debug(`Tool ${toolCall.toolName} executed successfully with result: ${JSON.stringify(result)}`);

        } catch (error: any) {
            this.logger.error(`Error executing tool ${toolCall.toolName} (ID: ${toolCall.id}):`, error);
